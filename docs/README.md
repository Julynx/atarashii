<p align="center">
  <img
    src="https://github.com/Julynx/atarashii/blob/main/assets/icons/app-icon.ico"
    width="200px"
  />
</p>

<h3 align="center">Atarashii</h3>
<p align="center">
  <i
    >A live Markdown to PDF editor with support for diagrams, custom CSS, and much more.</i
  >
</p>

<hr />

<p align="center">
  <a
    href="https://github.com/Julynx/atarashii/releases/latest/download/Atarashii_Setup.exe"
    target="_blank"
  >
    <img
      src="https://raw.githubusercontent.com/Julynx/julynx/refs/heads/main/assets/download.png"
      height="48"
    /> </a
  >&nbsp;&nbsp;<a
    href="https://github.com/Julynx/atarashii/releases/latest/download/Atarashii_Setup.appimage"
    target="_blank"
  >
    <img
      src="https://raw.githubusercontent.com/Julynx/julynx/refs/heads/main/assets/download-linux.png"
      height="48"
    />
  </a>
  <br /><br />
</p>

![alt text](https://github.com/Julynx/atarashii/blob/main/assets/images/screenshot.png)

## Features

- Live Markdown to PDF editing.
- Full [markdown-convert syntax](https://github.com/Julynx/markdown_convert/blob/main/CUSTOM_SYNTAX.md) support.
- Autosave, undo / redo, search, zoom in / out, line wrap, logging, printing...
