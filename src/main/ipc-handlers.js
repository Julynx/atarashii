...
const fs = require("fs");
const { ipcMain, dialog, shell, app, Menu, MenuItem, BrowserWindow } = require("electron");
 ...
 * @returns {void}
 */
function registerIpcHandlers(mainWindow, logger, consentStore, converterService) {
  ...
  ipcMain.handle("requirements:install", async () => {
    try {
      ...
      logger.error(`Requirements installation failure: ${installError.message}`);
  ...
  ipcMain.handle("project:get-defaults", () => {
    ...
    const nextAvailableProjectName = getNextAvailableProjectName(defaultParentDirectory);
  ...
  ipcMain.handle("project:create", async (_event, parentDirectory, projectName) => {
    try {
      const projectMetadata = createProject(parentDirectory, projectName, logger);
  ...
  ipcMain.handle("project:pick-and-open", async () => {
    ...
    try {
      const projectMetadata = validateAndOpenProject(selectedFolderPath, logger);
      return { ok: true, project: projectMetadata };
  ...
  ipcMain.handle("project:read-documents", (_event, projectPath, markdownFileName, cssFileName) => {
    try {
      const documents = readProjectDocuments(projectPath, markdownFileName, cssFileName);
  ...
  ipcMain.handle("project:save-document", async (_event, saveParameters) => {
    ...
    try {
      ...
      writeProjectDocument(projectPath, fileName, formattedText);
  ...
  ipcMain.handle("converter:start", async (_event, startParameters) => {
    const { projectPath, markdownFileName, cssFileName, pdfFileName } = startParameters;
    try {
